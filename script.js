...
// Allow emoji/Unicode messages end-to-end.
// Using textContent for rendering already prevents HTML injection, and modern browsers handle emoji in input values.
...
function initTheme() {
    ...
    // Default to dark (matches current CSS), but respect system preference if present.
...
async function handleUserInput() {
    ...
    if (userMessage) {
        ...
        try {
            // Add typing indicator (animated dots) before bot response
            ...
            typingMessageEl.appendChild(profileImage);
            typingMessageEl.appendChild(messageContent);
            chatMessages.appendChild(typingMessageEl);
            chatMessages.scrollTop = chatMessages.scrollHeight;

            // Simple animation using CSS class toggle
            let dotIndex = 0;
            const dotStates = ['.', '..', '...'];
            const typingTimer = setInterval(() => {
                dots.textContent = dotStates[dotIndex % dotStates.length];
                dotIndex++;
            }, 350);
